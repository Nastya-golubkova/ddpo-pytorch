...
**JPEG compressibility + aesthetic (e.g. 0.3 / 0.7):** preset `jpeg_aesthetic_hybrid` uses `jpeg_aesthetic_03_07` — each reward is z-scored within the batch, then combined so **0.7 of the mixture is aesthetic** (see `jpeg_aesthetic_combined` in `ddpo_pytorch/rewards.py`). Launch:
```bash
accelerate launch scripts/train.py --config config/dgx.py:jpeg_aesthetic_hybrid
```
To pick other weights, set `reward_fn = "jpeg_aesthetic_combined"` and `reward_fn_kwargs` with `jpeg_weight`, `aesthetic_weight`, and optionally `normalize_per_batch` in `config/base.py` (or your config module).
